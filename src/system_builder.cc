...
TransitionSystem * TransitionSystemBuilder::build(const po::variables_map & conf) {
  ...
  std::string root = conf["root"].as<std::string>();
  if (system_name == "arcstd") {
    sys = new ArcStandard(corpus.deprel_map, root);
  } else if (system_name == "arceager") {
    sys = new ArcEager(corpus.deprel_map, root);
  } else if (system_name == "archybrid") {
    sys = new ArcHybrid(corpus.deprel_map, root);
  } else if (system_name == "swap") {
    sys = new Swap(corpus.deprel_map, root);
  } else {
